...
public class IAutoWired {
   ...
   * 管理的类
   */
  static final Map<String, Object> beanList = new HashMap<>();
  ...
  private static void registered(String key, Object object) {
    ...
    if (list != null) {
      for (AutoWiredInject autoWiredInject : list) {
        autoWiredInject.setValue(object);
      }
      list.clear();
    }
    if (object instanceof Activity) {
      ...
      if (listView != null) {
        for (AutoWiredInjectView autoWiredInjectView : listView) {
          try {
  ...
  public static void inject(Object source) {
    ...
    injectHasActivity(source, activityKey, activityField, viewFields);
  }
  ...
  private static void injectAutoWired(Object source, List<Field> fields, List<Class<?>> clas, @SuppressWarnings("unused") Class<?> sourceClass) {
    for (Field field : fields) {
      AutoWired autowired = field.getAnnotation(AutoWired.class);
      if (autowired != null) {
        try {
